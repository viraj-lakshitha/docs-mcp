...
// Mounted at /api/tables in src/app.js, behind requireApiAuth (session
...
export function parseCsv(csv, hasHeaderRow) {
...
function inferType(values) {
  const sample = values.filter((v) => v !== undefined && v !== "").slice(0, TYPE_SAMPLE_SIZE);
...
export function tablesRouter() {
  ...
  router.post("/import", ah(async (req, res) => {
    ...
    for (let i = 0; i < headers.length; i++) {
      ...
      columns.push(await store.addColumn(req.userId, table.id, { name: headers[i] || `Column${i + 1}`, type }));
    }
  ...
  router.post("/:id/columns", ah(async (req, res) => {
    const { name, type } = req.body ?? {};
    if (!name || typeof name !== "string") return res.status(400).json({ error: "name is required" });
    const column = await store.addColumn(req.userId, req.params.id, { name, type });
    if (column === "invalid_type") return res.status(400).json({ error: "type must be one of: text, number, boolean, date" });
    column ? res.status(201).json(column) : res.status(404).json({ error: "not found" });
  }));

  router.patch("/:id/columns/:columnId", ah(async (req, res) => {
    const { name, type } = req.body ?? {};
  ...
  router.get("/:id/rows", ah(async (req, res) => {
  ...
  router.post("/:id/rows", ah(async (req, res) => {
    const { data } = req.body ?? {};
  ...
  router.get("/:id/rows/:rowId", ah(async (req, res) => {
    const row = await store.getRow(req.userId, req.params.id, req.params.rowId);
    row ? res.json(row) : res.status(404).json({ error: "not found" });
  }));

  router.patch("/:id/rows/:rowId", ah(async (req, res) => {
    const { data } = req.body ?? {};
    if (!data || typeof data !== "object") return res.status(400).json({ error: "data is required" });
  ...
  router.delete("/:id/rows/:rowId", ah(async (req, res) => {
  ...
  router.post("/:id/rows/import/preview", ah(async (req, res) => {
    const table = await store.getTable(req.userId, req.params.id);
  ...
  router.post("/:id/rows/import", ah(async (req, res) => {
    const table = await store.getTable(req.userId, req.params.id);
    if (!table) return res.status(404).json({ error: "not found" });
    const { csv, hasHeaderRow = true, columnMapping } = req.body ?? {};
    if (!columnMapping || typeof columnMapping !== "object") {
      return res.status(400).json({ error: "columnMapping ({csvHeader: columnId}) is required" });
    }
    const { headers, dataRows } = parseCsv(csv, hasHeaderRow);
    const columnsById = Object.fromEntries(table.columns.map((c) => [c.id, c]));
    ...
    const rows = dataRows.map((row, rowIndex) => {
      ...
      headers.forEach((header, i) => {
        ...
        const column = columnId && columnsById[columnId];
        if (!column) return;
    ...
    res.status(201).json({ imported, skipped: 0, errors });
  }));
