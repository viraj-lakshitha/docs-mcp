...
Every create/upload tool also returns a structuredContent block (id, and a
public url where applicable) alongside its Markdown text, for callers that
want to bind to fields directly instead of parsing prose.
...
export function buildServer(userId: string, traceId?: string): McpServer {
  ...
  // instrumentation to keep in sync as tools are added. The cast is
  // deliberate: a logging shim doesn't need to (and can't cleanly) preserve
  // registerTool's per-call generic input/output typing.
    ...
    registerTool(name, config as never, (async (...args: unknown[]) => {
      ...
      try {
        ...
        log("mcp.tool.result", {
          ...
          isError: !!(result as ToolContent | undefined)?.isError,
        });
        return result;
      } catch (err) {
        logError("mcp.tool.error", { traceId, userId, tool: name, ms: Date.now() - startedAt, error: (err as Error).message });
        throw err;
      }
  ...
  // `structured` is optional: when present it's returned as MCP
  // structuredContent alongside the Markdown text, so a programmatic caller
  // (an n8n workflow, a script) can read e.g. `output.id`/`output.url`
  // directly instead of parsing the id out of the prose.
  const md = (text: string, structured?: Record<string, unknown>): ToolContent => ({
    content: [{ type: "text", text }],
    ...(structured ? { structuredContent: structured } : {}),
  });
  const error = (message: string): ToolContent => ({
    content: [{ type: "text", text: message }],
    isError: true,
  });

  const editorUrl = (id: string): string => `${store.baseUrl()}/app#${id}`;

  const docPreview = (doc: Document, verb: string): ToolContent => {
    const parts = [
    ...
    return md(parts.join("\n"), { id: doc.id, editor_url: editorUrl(doc.id) });
  };

  const docTable = (docs: { id: string; title: string; updated_at: string }[]): ToolContent => {
  ...
  server.registerTool(
    ...
    async () => ({ content: [{ type: "text" as const, text: INSTRUCTIONS }] })
  );
  ...
  server.registerTool(
    "create_document",
    {
      ...
      outputSchema: { id: z.string(), editor_url: z.string() },
    },
  ...
  server.registerTool(
    "upload_asset",
    {
      ...
      outputSchema: { id: z.string(), url: z.string() },
    },
    async ({ filename, mime_type, base64_data }: { filename: string; mime_type: string; base64_data: string }) => {
      ...
      try {
        ...
        return error(`Asset upload failed: ${(err as Error).message}`);
      }
    }
  );
  ...
  server.registerTool(
    ...
    async ({ id }: { id: string }) => {
      ...
      return md(
        ...
        { token: share.token, url: share.url }
      );
    }
  );
  ...
  const tableList = (tables: { name: string; column_count: number; row_count: number; id: string }[]): ToolContent => {
  ...
  const columnList = (columns: TableColumn[]): string => {
    if (columns.length === 0) return "_(no columns yet)_";
    return columns.map((c) => `- **${c.name}** (${c.type}) — id \`${c.id}\``).join("\n");
  };

  const tableGrid = (columns: TableColumn[], rows: TableRow[]): ToolContent => {
  ...
  const resolveRowData = (columns: TableColumn[], data: Record<string, unknown> | undefined): Record<string, unknown> => {
  ...
  server.registerTool(
    ...
    async ({
      ...
      const full = (await store.getTable(userId, table.id)) as DataTableWithColumns;
      return md(
  ...
  server.registerTool(
    ...
    async ({ table_id, name, description }: { table_id: string; name?: string; description?: string }) => {
      const table = await store.updateTable(userId, table_id, { name, description });
      return table
        ? md(`Updated table **${table.name}** — id \`${table.id}\``, { id: table.id })
        : error(`No table with id ${table_id}`);
    }
  );
  ...
  server.registerTool(
    ...
    async ({ table_id }: { table_id: string }) =>
      (await store.deleteTable(userId, table_id))
        ? md(`Deleted table \`${table_id}\`.`)
        : error(`No table with id ${table_id}`)
  );
  ...
  server.registerTool(
    "add_column",
    {
      ...
      outputSchema: { id: z.string(), table_id: z.string() },
    },
    async ({ table_id, name, type }: { table_id: string; name: string; type: ColumnType }) => {
      ...
      return column
        ? md(`Added column **${column.name}** (${column.type}) — id \`${column.id}\``, { id: column.id, table_id })
        : error(`No table with id ${table_id}`);
    }
  );
  ...
  server.registerTool(
    ...
    async ({
      ...
      if (column === "invalid_type") return error("type must be one of: text, number, boolean, date");
      return column
        ? md(`Updated column **${column.name}** (${column.type}).`, { id: column.id, table_id })
  ...
  server.registerTool(
    ...
    async ({ table_id, column_id }: { table_id: string; column_id: string }) =>
  ...
  server.registerTool(
    ...
    async ({ table_id, limit, offset }: { table_id: string; limit?: number; offset?: number }) => {
      const table = await store.getTable(userId, table_id);
      if (!table) return error(`No table with id ${table_id}`);
      const result = await store.listRows(userId, table_id, { limit, offset });
      if (!result) return error(`No table with id ${table_id}`);
  ...
  server.registerTool(
    "create_row",
    {
      ...
      inputSchema: {
        table_id: z.string().describe("Table id"),
        data: z.record(z.any()).describe("Row data keyed by column name"),
      },
      outputSchema: { id: z.string(), table_id: z.string() },
    },
    async ({ table_id, data }: { table_id: string; data: Record<string, unknown> }) => {
      const table = await store.getTable(userId, table_id);
      if (!table) return error(`No table with id ${table_id}`);
      const row = await store.createRow(userId, table_id, resolveRowData(table.columns, data));
      if (!row) return error(`No table with id ${table_id}`);
      return md(`Created row \`${row.id}\` in **${table.name}**.`, { id: row.id, table_id });
    }
  );
  ...
  server.registerTool(
    ...
    async ({ table_id, row_id, data }: { table_id: string; row_id: string; data: Record<string, unknown> }) => {
      const table = await store.getTable(userId, table_id);
      if (!table) return error(`No table with id ${table_id}`);
      const row = await store.updateRow(userId, table_id, row_id, resolveRowData(table.columns, data));
      return row ? md(`Updated row \`${row.id}\`.`, { id: row.id, table_id }) : error(`No row with id ${row_id}`);
    }
  );
  ...
  server.registerTool(
    ...
    async ({ table_id, row_id }: { table_id: string; row_id: string }) =>
      (await store.deleteRow(userId, table_id, row_id))
        ? md(`Deleted row \`${row_id}\`.`)
        : error(`No row with id ${row_id}`)
  );
  ...
  server.registerTool(
    "import_csv_rows",
    {
      ...
      outputSchema: { table_id: z.string(), imported: z.number(), skipped: z.number(), errors: z.array(z.string()) },
    },
    async ({ table_id, csv, has_header }: { table_id: string; csv: string; has_header?: boolean }) => {
      const table = await store.getTable(userId, table_id);
      if (!table) return error(`No table with id ${table_id}`);
      ...
      const imported = (await store.bulkInsertRows(userId, table_id, rows)) ?? 0;
      const parts = [`Imported ${imported} row(s) into **${table.name}**.`];
      if (errors.length > 0) {
        parts.push("", `${errors.length} cell(s) couldn't be coerced and were left blank:`, ...errors.slice(0, 20).map((e) => `- ${e}`));
      }
      return md(parts.join("\n"), { table_id, imported, skipped: 0, errors });
    }
  );
