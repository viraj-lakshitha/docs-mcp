...
import type { ColumnType } from "../../../../shared/types.ts";
...
export interface CreateTableSubmission {
  name: string;
  description?: string;
  columns: ColumnDraft[];
}
...
export function CreateTableDialog({
  ...
  onSubmit: (req: CreateTableSubmission) => Promise<unknown>;
  onClose: () => void;
}) {
        ...
        <div className="column-editor">
          {columns.map((col, i) => (
            <div className="column-editor-row" key={i}>
              ...
              />
              <select className="input" value={col.type} onChange={(e) => updateColumn(i, { type: e.target.value as ColumnType })}>
                {COLUMN_TYPES.map((t) => (
