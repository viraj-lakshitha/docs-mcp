...
export function CsvImportDialog({ tableId, columns, onImported, onClose }) {
  const [step, setStep] = useState("pick"); // pick -> map (existing-table mode only) -> done
  const [fileName, setFileName] = useState("");
  const [csv, setCsv] = useState("");
  const [hasHeaderRow, setHasHeaderRow] = useState(true);
  const [name, setName] = useState("");
  const [preview, setPreview] = useState(null); // {headers, sample, rowCount}
  const [mapping, setMapping] = useState({}); // csvHeader -> columnId
  ...
  const onFileChange = async (e) => {
    ...
    if (tableId) {
      ...
      setError("");
      try {
        const p = await api("POST", `/api/tables/${tableId}/rows/import/preview`, { csv: text, hasHeaderRow });
        setPreview(p);
  ...
  const submitCreate = async () => {
    ...
    setBusy(true);
    try {
      const res = await api("POST", "/api/tables/import", { name: name.trim(), csv, hasHeaderRow });
      setResult(res);
      setStep("done");
  ...
  const submitImport = async () => {
    ...
    setBusy(true);
    try {
      const res = await api("POST", `/api/tables/${tableId}/rows/import`, { csv, hasHeaderRow, columnMapping: mapping });
      setResult(res);
      setStep("done");
      onImported(null);
    } catch (err) {
