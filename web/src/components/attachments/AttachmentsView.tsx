...
import type { AssetSummary } from "../../../../shared/types.ts";
import { Card } from "../Card.tsx";
import { Button } from "../Button.tsx";
import { EmptyState } from "../EmptyState.tsx";
import { ConfirmDialog } from "../Dialog.tsx";
import { PaperclipIcon, XIcon } from "../Icons.tsx";
...
export function AttachmentsView() {
  const [assets, setAssets] = useState<AssetSummary[] | null>(null); // null = loading
  const [dialog, setDialog] = useState<AssetSummary | null>(null);
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [error, setError] = useState("");
  const [uploading, setUploading] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  const refresh = () => api<AssetSummary[]>("GET", "/api/assets").then(setAssets);
  useEffect(() => { refresh(); }, []);
  ...
  const copyLink = async (asset: AssetSummary) => {
